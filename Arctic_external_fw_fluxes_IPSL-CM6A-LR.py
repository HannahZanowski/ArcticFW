...
##############Arctic_external_fw_fluxes-IPSL-CM6A-LR.py#################
#Compute river and p-e fw fluxes into Arctic Ocean
...
xda=xr.open_dataset(direca_static+'areacella/gr/v20181123/areacella/areacella_fx_'+model+'_piControl_r1i1p1f1_gr.nc')
...
xdo=xr.open_dataset(direco_static+'areacello/gn/v20181123/areacello/areacello_Ofx_'+model+'_piControl_r1i1p1f1_gn.nc')
...
for ens_num in ens_nums:
	...
	#Atmo directory for IPSL-CM6A-LR for CMIP6
	...
	riv_path=direc_o+'friver/gn/'+sorted(os.listdir(direc_o+'friver/gn/'))[-1]+'/friver/friver_Omon_'+simulation+time_period+'.nc'
	...
	#Get river flux
	xdo=xr.open_dataset(riv_path,chunks={'time':1})
	riv=xdo['friver'][:,:-1,1:-1] #river flux into ocean [kg/m2/s]
	...
	pr=xda['pr'].sel({'lat':slice(50,None)}) #precp flux (solid and liquid) into atmo [kg/m2/s], N of 50N
